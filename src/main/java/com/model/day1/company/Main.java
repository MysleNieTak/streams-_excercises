...
// 1. Wylistuj (system out println) wszystkie firmy
        ...
        company_3_london_by_emoplyees_ascending(companies);
        ...
        company_5_kijow_company_by_employeenumber(companies);

        // 6. Zwróć firmę z najkrótszą nazwą

        company_6_company_with_shortestname(companies);

        // 7. Zwróć firmę która nie pochodzi z Kijowa, Londynu i Detroit, która ma najmniej kupionych produktów.

        company_7_not_from_KijowLondonDetroit_lowest_number_of_products(companies);

        // 8. Każdej firmie dodaj po 1 pracowniku, jeśli pochodzi z Kijowa lub Detroit

        company_8_add_1_employee_if_from_Kijow_or_Detroit(companies);
        ...
        company_9_map_by_companyName_vale_is_number_of_employees(companies);
        ...
        company_11_company_largest_amount(companies);
...
company_15_najwiecej_kawy(companies);
    }
...
// 1. Wylistuj (system out println) wszystkie firmy
    ...
    private static void company_3_london_by_emoplyees_ascending(List<Company> companies) {
    ...
    private static void company_4_warszawa_purchases_and_employees(List<Company> companies) {
        Set<String> result4 = companies.stream()
                ...
                .sorted((f1, f2) -> {
                    int wynikPorownaniaIlosciZakupow = Integer.compare(f1.getPurchaseList().size(), f2.getPurchaseList().size());
                    if (wynikPorownaniaIlosciZakupow == 0) {
                        return Integer.compare(f2.getEmployees(), f1.getEmployees());
                    }
                    return wynikPorownaniaIlosciZakupow;
                })
                .map(Company::getName)
                .collect(Collectors.toSet());
    ...
    private static void company_5_kijow_company_by_employeenumber(List<Company> companies) {
        Optional<Company> result5 = companies.stream()
                .filter(company -> company.getCityHeadquarters().equals("Kijow"))
    ...
    private static void company_6_company_with_shortestname(List<Company> companies) {
    ...
    private static void company_7_not_from_KijowLondonDetroit_lowest_number_of_products(List<Company> companies) {
        Optional<Company> result7 = companies.stream()
                //           .filter(company -> !company.getCityHeadquarters().equals("London"))
                //         .filter(company -> !company.getCityHeadquarters().equals("Kijow"))
                //       .filter(company -> !company.getCityHeadquarters().equals("Detroit"))
                .filter(company -> !List.of("Kijow", "London", "Detroit").contains(company.getCityHeadquarters()))
    ...
    private static void company_8_add_1_employee_if_from_Kijow_or_Detroit(List<Company> companies) {

        companies.stream()
                .filter(company -> company.getCityHeadquarters().equals("Kijow") || company.getCityHeadquarters().equals("Detroit"))
                .forEach(company -> {
    ...
    // 9. ** Zwróć MAPĘ w której kluczem jest nazwa firmy, a wartością ilość pracowników w tej firmie (https://howtodoinjava.com/java8/collect-stream-to-map/)
    private static void company_9_map_by_companyName_vale_is_number_of_employees(List<Company> companies) {
        Map<String, Integer> result9 =companies.stream()
                .collect(Collectors.toMap(
                        // czesc 1 - skąd mają się wziąć klucze
                        company -> company.getName(),

                        // część 2 - skąd mają sięwziąc wartości
                        company -> company.getEmployees(),
                        // część 3 - co zrobić, jeśli mamy 2 obiekty pod tym samym kluczem
                        (o, o2) -> {
                            //jeśli trafi się firma o tej samej nazwie, to ilość pracowników ma się zsumować
                            return o + o2;
                        }
                ));
...
// 11. Zwróć firmę która dokonała zakupów na największą kwotę
private static void company_11_company_largest_amount(List<Company> companies) {
...
//        company_12_zakupy_10k(companies);
    ...
    private static void company_12_zakupy_10k(List<Company> companies) {
        ...
        Optional<Company> result12 = companies.stream().max((o1, o2) -> {
 ...
 private static void company_13_najwiecej_hajsu(List<Company> companies){
    ...
    private static void company_15_najwiecej_kawy(List<Company> companies) {
